...
jQuery(document).ready(function() {
      ...
      if(!jQuery('body').hasClass('leftpanel-collapsed')) {
         if(sub.is(':visible')) {
            sub.slideUp(200, function(){
               ...
               jQuery('.mainpanel').css({height: 'auto'});
 ...
 jQuery('.mainpanel').css({'minHeight':winNewHeight, 'height':'auto'});
   }
   adjustmainpanelheight();
...
// added by Hetal Patel
 function ImageTypeValidation(id,val)
 {
     var inputFile = val;
    var fileExtension = ['jpeg', 'jpg','png','gif'];
    if ($.inArray(inputFile.split('.').pop().toLowerCase(), fileExtension) == -1) {
         myAlert("Only '.jpg','.gif' or '.png' formats are allowed.");
         $('#'+id).val('');
         var fileType = $('#'+id);
         fileType.replaceWith(fileType.val('').clone(true));

     }
 }
 
 /**
  * @desc Validation Of Image Type and Dimention
  */
 function validateImage(data, width, height) {
    var _URL = window.URL || window.webkitURL;
    var image, file, name, ratio, fileext, newratio, expectration, newexpectration;
    name = $(data).val();
    ratio = height/width;
    newratio = ratio.toFixed(2);
    fileext = name.split('.').pop();
    fileext=fileext.toLowerCase();
    if(fileext != 'jpg' && fileext != 'jpeg' && fileext != 'gif' && fileext != 'png' && fileext != 'bmp'){
          myAlert('Please Select Image with jpg,jpeg,gif,png extension.');
          $(data).val('');
    }else{
        if (file = data.files[0]) {
            image = new Image();
            image.onload = function() {
                expectration = this.height/this.width;
                newexpectration = expectration.toFixed(2);
                if(this.width < width && this.height < height || newexpectration != newratio){
                    $(data).val('');
                    myAlert("Invalid Dimension, Upload Minimum Dimension (" + width + " x " + height + ") and Ratio should be (1 x " + newratio + ")");
...
/**
  * @desc Validation Of Image Type,Dimention and Submit Form
  */
 function validateImageSubmit(data, width, height,formId) {
    var _URL = window.URL || window.webkitURL;
    var image, file, name, ratio, fileext, newratio, expectration, newexpectration;
    name = $(data).val();
    fileext = name.split('.').pop();
        ...
        if (file = data.files[0]) {
            ...
            image.onload = function() {
                if(this.width < width && this.height < height){
                    $(data).val('');
                    myAlert("Invalid Dimension, Upload Minimum Dimension (" + width + " x " + height + ")");
