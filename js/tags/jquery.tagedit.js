	...
	$.fn.tagedit = function(options) {
		...
		*/
		...
		if(baseName && baseName.length == 4) {
			...
			myAlert('elementname dows not match the expected format (regexp: '+baseNameRegexp+')')
			return;
		}
					...
					switch(event.target.tagName) {
						case 'A':
							$(event.target).parent().fadeOut(options.animSpeed, function() {
                                                                // code added to remove specific index for movie sectino:rajesh
                                                                var timeVal = $(this).find('span').html();
                                                                if(typeof(arr) != 'undefined')
                                                                {
                                                                    var index=arr.indexOf(timeVal);
                                                                    arr.splice(index, 1);
